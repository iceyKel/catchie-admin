...
function editProd(str) {
    if (str == "") {
        ...
        xmlhttp.open("GET","sql_commands/edit_prod.php?q="+str,true);
        xmlhttp.send();
    }
}
...
<nav class="navbar navbar-inverse">
  <div class="container-fluid">
    ...
    <ul class="nav navbar-nav">
      <li><a href="manage_orders.php">Inbox</a></li>
...
<div class="body">
  ...
  <tbody>
   ...
   include 'config.php';
        ...
        $sql_fetch_prd = "SELECT tblproducts.product_id,tblproducts.prod_name,tblproducts.description,tblproducts.qty,tblproducts.price,tblstore.store_name FROM tblproducts INNER JOIN tblstore ON tblproducts.store_id = tblstore.store_id";
        $result = mysqli_query($conn, $sql_fetch_prd);
          if (mysqli_num_rows($result) > 0) {
              while($row = mysqli_fetch_assoc($result)) {
               ...
               $prod_des = $row['description'];
               $qty = $row['qty'];
               $price = $row['price'];
               $store = $row['store_name'];                   

             // echo "<form action='../sql_commands/.php' method = 'POST'>";
              ...
              echo "<td>".$store."</td>";
              echo "<td>
                        <center><input type = 'hidden' name='prod_id' value='".$prod_id."'>
                       
                        <input type = 'hidden' name='prod_id' value='".$prod_id."'>
                        <button data-toggle='modal' data-target='#myModal_adding' id = 'button3' class= 'btn btn-info' type = 'submit' name='submit' value='".$prod_id."' onclick ='editProd(this.value)'>Edit</button></center>
                    </td>
                    </tr>";
             // echo "</form>";
